...
function SofFilter(query) {
  const self = this;
  if (!(self instanceof SofFilter)) { throw new Error('the SofFilter object must be instantiated with `new` flag.. (e.g. `const filters = new SofFilter(query);`)'); }
  if (typeof query !== 'object') { throw new Error('the SofFilter object must be instantiated with an SofQuery instance (e.g. `const filters = new SofFilter(new SofQuery(provider));`). See octonion.institute/susy-js/sofjs-query for more details..'); }
