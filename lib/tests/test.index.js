...
var provider = Susybraid.provider();
var Sof = require('sofjs-query');
var SofFilter = require('../index.js');
var assert = require('chai').assert;
var sha3 = require('sofjs-sha3'); // eslint-disable-line
var abi = require('sofjs-abi'); // eslint-disable-line
console.warn = function warn() {}; // eslint-disable-line

describe('SofFilter', function () {
  describe('constructor', function () {
    it('should construct properly', function () {
      var sof = new Sof(provider);
      var filters = new SofFilter(sof);
    ...
    it('should throw under bad construction', function () {
      assert.throws(function () {
        new SofFilter(4354); // eslint-disable-line
      }, Error);
      assert.throws(function () {
        SofFilter({}); // eslint-disable-line
      }, Error);
      assert.throws(function () {
        SofFilter(); // eslint-disable-line
      }, Error);
  ...
  describe('Filter', function () {
    ...
    it('should setup a new filter and handle error', function (done) {
      ...
      FakeProvider.prototype.sendAsync = function sendAsync(payload, callback) {
        ...
        if (payload.method === 'sof_newFilter') {
          var fakeEventLog = {
            id: payload.id,
      ...
      var sof = new Sof(new FakeProvider());
      var filters = new SofFilter(sof);
      var filter = new filters.Filter();
      filter['new']({ fromBlock: 0 }, function (error, result) {
    ...
    it('should setup a uninstall filter and handle error', function (done) {
      ...
      FakeProvider.prototype.sendAsync = function sendAsync(payload, callback) {
        ...
        if (payload.method === 'sof_uninstallFilter') {
          var fakeEventLog = {
            id: payload.id,
            jsonrpc: payload.jsonrpc,
            error: 'invalid data!',
