...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("SofFilter", [], factory);
	else if(typeof exports === 'object')
		exports["SofFilter"] = factory();
	else
		root["SofFilter"] = factory();
})(this, function() {
        ...
        query.getFilterChanges(self.filterId, function (changeError, changeResult) {
          ...
          if (!changeError) {
            try {
              ...
              decodingError = new Error('[sofjs-filter] while decoding filter change event data from RPC \'' + JSON.stringify(decodedChangeResults) + '\': ' + decodingErrorMesage);
            }
          }
...
/**
 * SofFilter constructor, intakes a query, helps manage filter event polling
 *
 * @method SofFilter
 * @param {Object} query the `sofjs-query` or `sof-query` object
 * @returns {Object} output an SofFilter instance
 * @throws error if new is not used
 */

function SofFilter(query) {
  var self = this;
  if (!(self instanceof SofFilter)) {
    throw new Error('the SofFilter object must be instantiated with `new` flag.. (e.g. `const filters = new SofFilter(query);`)');
  }
  if (typeof query !== 'object') {
    throw new Error('the SofFilter object must be instantiated with an SofQuery instance (e.g. `const filters = new SofFilter(new SofQuery(provider));`). See octonion.institute/susy-js/sofjs-query for more details..');
  }
...
// export SofFilter
module.exports = SofFilter;

/***/ }
/******/ ])
});
;
//# sourceMappingURL=sofjs-filter.js.map